//
// This program calculates quantum transition amplitudes
// for the transverse-field Ising model Hamiltonian using divided differences
//
...
#define qmax 4        // maximal addition to the initial expansion order, i.e., Q = Ndifferentbits + qmax
...
std::complex<double> beta(0,1);        // beta = I*t
std::complex<double> Gamma(0.001,0);

unsigned long long zi = 16210525687446977967ULL; // index of canonical basis vector z_i
unsigned long long zf = 16489678495598722447ULL; // index of canonical basis vector z_f
...
void sequences(){ // for a given k_0,...,k_{n-1} consider all paths such that number of flips of spin #i is k_i.
	int i, prevEnergy; std::complex<double> z;
	if(currq<q){ for(i=0;i<n;i++) if(currk[i]>0){
		currk[i]--; currseq[currq]=i; currq++;
		prevEnergy=currEnergy; currEnergy+=flipDeltaE(i);
		z = -beta*(double)currEnergy; d->AddElement(z,currs);
		sequences();
...
int main(int argc, char** argv){
	...
	divdiffcomplex dd(qmax+Ndifferentbits+4,500); d = &dd; currEnergy=IsingEnergy();
		...
		dd.CurrentLength=0; z = -beta*(double)currEnergy; dd.AddElement(z,currs);
