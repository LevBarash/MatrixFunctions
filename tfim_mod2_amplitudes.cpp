...
// for the transverse-field Ising model Hamitlonan modulo two using divided differences
//
...
std::complex<double> Gamma(0.05,0);
...
std::complex<ExExFloat> divdiffs[qmax+n+2]; std::complex<ExExFloat> coefficient; ExExFloat Tolerance;
...
void fill_divdiffs(){
	int i, E1=3;
	divdiffcomplex d(qmax+Ndifferentbits+4,500); std::complex<ExExFloat> coeff(1,0);
	for(E1=0;E1<=q+1;E1++){
...
int main(int argc, char** argv){
	...
	std::complex<ExExFloat> sum,totalsum,summand; divdiff_init(); Tolerance = tolerance;
